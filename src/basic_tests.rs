...
fn test_seq() {
    ...
    s.send_enc(b"World".to_vec(), false);
    s.send_clr(b"foo".to_vec(), false);
    s.ratchet(32, false);
    s.recv_clr(b"bar".to_vec(), false);
    s.recv_enc(b"baz".to_vec(), false);
    for i in 0..100 {
        s.send_enc(vec![b'X'; i], false);
    }
    s.prf(123, false);
    s.send_mac(16, false);
...
fn test_metadata() {
    ...
    let mut md = s.meta_send_clr(b"meta1".to_vec(), false);

    // This does not output anything
    s.key(b"key".to_vec(), false).unwrap();

    md.extend(s.meta_prf(10, false));
    md.extend(s.prf(10, false));
    md.extend(s.meta_send_clr(b"meta3".to_vec(), false));
    md.extend(s.send_enc(b"pt".to_vec(), false));
...
fn test_streaming() {
    ...
    let one_shot_st: Vec<u8> = {
        ...
        s.ad(b"mynonce".to_vec(), false);
        s.recv_enc(b"hello there".to_vec(), false);
        s.send_mac(16, false);
        s.st.0.to_vec()
    };
    ...
    let streamed_st: Vec<u8> = {
        let mut s = Strobe::new(b"streamingtest".to_vec(), SecParam::B256);
        s.ad(b"my".to_vec(), false);
        s.ad(b"nonce".to_vec(), true);
        s.recv_enc(b"hello".to_vec(), false);
        s.recv_enc(b" there".to_vec(), true);
        s.send_mac(10, false);
        s.send_mac(6, true);
        s.st.0.to_vec()
    };
...
fn test_enc_correctness() {
    ...
    tx.key(b"the-combination-on-my-luggage".to_vec(), false);
    rx.key(b"the-combination-on-my-luggage".to_vec(), false);

    let ciphertext = tx.send_enc(orig_msg.clone(), false);
    let decrypted_msg = rx.recv_enc(ciphertext, false);
...
fn test_mac_correctness() {
    ...
    tx.key(b"secretsauce".to_vec(), false);
    let ct = tx.send_enc(b"attack at dawn".to_vec(), false);
    let mac = tx.send_mac(16, false);

    rx.key(b"secretsauce".to_vec(), false);
    rx.recv_enc(ct, false);

    // Test that valid MACs are accepted
    let mut rx_copy = rx.clone();
    let good_res = rx_copy.recv_mac(mac.clone(), false);
    ...
    let bad_res = rx.recv_mac(bad_mac, false);
